...
void info()
{
    ...
    foreach (string fileName in fileEntries)
    {
        ...
        if (extension == ".txt")
        {
            string[] lines = null;// = File.ReadAllLines(fileName);
        ...
        foreach (var file2 in lastCommitDir)
        {
            ...
            if (fname == fname2 && extension == extension2)
            {
                ...
                if (extension == ".txt")
                {
                    ...
                    int f2characters = 0;
                    //Console.Write($"{fname}: ");
                    //foreach(var line in linesf1)
                    //{
                    //    f1data.Add(line);
                    //    f1words += line.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries).Length;
                    //    f1characters += line.Length;
                    //}
                    //foreach(var line in linesf2)
                    //{
                    //    f2data.Add(line);
                    //    f2words += line.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries).Length;
                    //    f2characters += line.Length;
                    //}
                    ...
                    if (f1data.Count > f2data.Count)
                    {
                        ...
                        for (int i = 0; i < f2data.Count; i++)
                        {
                            ...
                            {
                                modified = true;
                                Console.WriteLine($"- The text starting with line {i + 1} has been modified");
                            }
                        }
                        Console.WriteLine($"- There have been added {f1data.Count - f2data.Count} lines of text");
                    }
                    else if (f1data.Count < f2data.Count)
                    {
                        ...
                        Console.WriteLine($"- There have been removed {f2data.Count - f1data.Count} lines of text");
                    }
                    else
                    {
                ...
                else if (extension == ".cpp")
                {
                    ...
                    //    if (line.Contains("class"))
                    //    {
                    //        f1classes.Add(line.Split(' ').Skip(1).FirstOrDefault());
                    //    }
                    //    if (line.Contains("{"))
                    //    {
                    //        f1methods++;
                    ...
                    //    if (line.Contains("class"))
                    //    {
                    //        f2classes.Add(line.Split(' ').Skip(1).FirstOrDefault());
                    //    }
                    //    if (line.Contains("{"))
                    //    {
                    //        f2methods++;
                    ...
                    if (f1data.Count > f2data.Count)
                    {
                        ...
                        for (int i = 0; i < f2data.Count; i++)
                        {
                            ...
                            {
                                modified = true;
                                Console.WriteLine($"- The code from line {i + 1} has been modified");
                            }
                        }
                        Console.WriteLine($"- There have been added {f1data.Count - f2data.Count} lines of code");
                    }
                    else if(f1data.Count < f2data.Count)
                    {
                        Console.Write($"{fname}: ");
                        ...
                        Console.WriteLine($"- There have been removed {f2data.Count - f1data.Count} lines of code");
                    }
                    else
                    {
                        bool nameshowed = false;
                    ...
                    {
                        Console.WriteLine($"- There have been added {f1methods - f2methods} methods.");
                    }
                    else if (f2methods > f1methods)
                    {
                        Console.WriteLine($"- There have been removed {f2methods - f1methods} methods.");
                    }
                }
            }
        }
...
void backToSnapshot()
{
    ...
    if (!returned)
    {
        Console.WriteLine("This snapshot does not exists.");
    }
